...
Headlines: the saving-rate \emph{trend} has stepped up about $2.6$ pp
($12.5\%\!\to\!15.1\%$, with the COVID forced-saving quarters excluded from the
filter) and the cycle is now near zero, so the persistence is mostly
...
(structural) and a cycle (Figure~\ref{fig:structural_vs_cyclical}). Because the
...
\fig{0.92}{structural_vs_cyclical}{The euro-area saving rate split into a structural
...
\fig{0.78}{europe_saving_map}{Gross household saving rate by country, 2024 --- the
...
The time dimension confirms the pattern (Figure~\ref{fig:saving_rate_evolution}): the
...
\fig{0.92}{saving_rate_evolution}{Household saving rates over time: all 21 countries
...
once goods and services are split (Figure~\ref{fig:goods_vs_services}). A high saving
...
\emph{risk} rather than liquidity (Figure~\ref{fig:saving_composition_evolution}). The
...
cross-country snapshot (Figure~\ref{fig:risky_share_by_country}) shows the risky share
is higher in the core/North and lower in the South.

\fig{0.86}{saving_composition_evolution}{Household financial assets by RISK over time:
