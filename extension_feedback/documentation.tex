...
energy-price liquidity-adequacy analysis. All series are euro-area and free
(Eurostat, the ECB Data Portal, FRED, and the Caldara--Iacoviello GPR index);
the scripts reuse the main project's helpers. Headline result: the broad
...
liquidity / maturity ladder (Table~\ref{tab:ladder}) and recompute the
...
T2 --- near-money ($\approx M2/M3$) & F29 other deposits (term \& redeemable at
  notice), F521 money-market-fund shares &
  accessible at short notice / maturity, minimal price risk \\
T3 --- marketable & F3 debt securities, F511 listed shares, F522 non-MMF
  investment-fund shares &
  sellable within days at market price (carries price risk) \\
...
\paragraph{Status.} This script requires FRED, which was unreachable from the
build environment, so Figure~\ref{fig:risk_premia_vs_gpr} regenerates when
\code{python3 risk\_premia.py} is run on a machine with FRED access; the FRED
identifiers are standard and identical in form to the project's existing FRED
pulls. Expected reading: positive correlations and a clear upward jump after
February 2022 --- geopolitical tension is observably priced. This is an
\emph{enabling} fact (the uncertainty is real and measurable in markets) rather
than direct evidence on the household saving motive.
...
\begin{itemize}
  ...
  \item \textbf{Pending data.} The risk-premia figure regenerates on a machine
  with FRED access. All series are pulled live and revised --- regenerate before
  citing.
\end{itemize}
