...
\paragraph{How the models are ranked.} The ``rates+inflation'' and ``precaution''
models are \emph{non-nested} --- neither is a special case of the other --- so
they cannot be compared with an ordinary $F$-test. We use the Akaike Information
Criterion,
\begin{equation}
\mathrm{AIC}_m = 2k_m - 2\ln \hat{L}_m,
\end{equation}
where $k_m$ is the number of parameters and $\hat L_m$ the maximised likelihood of
model $m$; lower is better (it rewards fit and penalises extra parameters). Raw
AIC values are not interpretable alone, so we convert them to \emph{Akaike
weights},
\begin{equation}
\Delta_m = \mathrm{AIC}_m - \min_j \mathrm{AIC}_j, \qquad
w_m = \frac{\exp(-\tfrac12\Delta_m)}{\sum_j \exp(-\tfrac12\Delta_j)} .
\end{equation}
Each $w_m$ is the probability that model $m$ is the best (Kullback--Leibler) model
in the candidate set --- exactly the ``how much more likely is rates/inflation
than precaution?'' statement we want.

\subsection{Results}
Table~\ref{tab:compecon} reports the three models for the tilt (annual levels,
$n=24$).

\begin{table}[htbp]\centering
\caption{Composition tilt on standardised drivers (annual, 2002--2025, $n=24$;
HC3 robust SEs). Coefficients in EUR bn per one-standard-deviation of the driver;
$^{*}$ denotes significance at 5\%. Akaike weights are computed across the three
models. Inflation enters positively but is insignificant throughout.}
\label{tab:compecon}
\renewcommand{\arraystretch}{1.2}
\begin{tabular}{@{}lccccc@{}}
\toprule
model & adj.\ $R^2$ & AIC & rate ($\beta_1$) & uncertainty ($\beta_3$) & Akaike weight \\
\midrule
rates $+$ inflation & 0.74 & 329.8 & $+278^{*}$ & --- & 25\% \\
precaution (GPR)    & 0.02 & 360.5 & --- & $+103$ & $\approx 0\%$ \\
all three           & 0.77 & 327.5 & $+304^{*}$ & $-96$ & 75\% \\
\midrule
\multicolumn{6}{@{}l}{\footnotesize head-to-head (rates$+$inflation vs.\ precaution): $\approx 100\%$ vs.\ $\approx 0\%$}\\
\bottomrule
\end{tabular}
\end{table}

The reading is unambiguous. The ECB rate is the dominant driver: a
one-standard-deviation rate increase shifts the tilt by roughly +EUR 280--300 bn
toward yield, significant at 5\%. The precaution model is almost empty (adjusted
$R^2 \approx 0.02$), and its uncertainty coefficient has the \emph{wrong} sign for
precaution and is insignificant. Head-to-head, the Akaike weights put essentially
$100\%$ on rates+inflation versus $0\%$ on precaution. Using net bond purchases as
the dependent variable gives the same verdict (rate coefficient $+$, $p=0.01$;
$100\%$ vs.\ $0\%$); there, uncertainty looks significant \emph{on its own} but
collapses to zero once the rate is included --- a textbook confounder, with
uncertainty merely standing in for rates.
...
annual changes; the model \emph{ranking} is nonetheless unchanged --- precaution
still receives essentially zero Akaike weight. Two caveats bound the claim. The
