#!/usr/bin/env python3
"""
Precautionary saving in Europe — STEP 3: econometrics
=====================================================

Does uncertainty actually drive euro-area precautionary saving? Run after
01_collect_data.py, which writes the CSVs this reads from ./data:
...
------------
  ...
  2. ARDL bounds test (Pesaran-Shin-Smith) for a long-run level relationship —
     valid for a mix of I(0) and I(1) regressors (Johansen kept only as a
     reference, valid only if every series is I(1)).
  3. Granger causality and a VAR/IRF in the form each series needs — LEVELS for
     the stationary core (saving, uncertainty), so no over-differencing.
  4. Re-runs the key tests on a post-2010 subsample.
  5. PROXY & TIMING: lead/lag cross-correlation of saving with GPR and EPU, to
     pick the proxy/lag that best tracks saving (folded in from the old
     proxy_timing_test.py).

Outputs:
    ./data/econometrics_results.md
    ./figures/A2_irf_saving_to_uncertainty.png
    ./figures/A3_proxy_leadlag.png

Run
---
    python 03_econometrics.py                 # GPR
    python 03_econometrics.py --proxy epu     # EU EPU
    python 03_econometrics.py --substart 2010 # change subsample start
"""
...
MAXK = 8  # quarters of lead/lag to scan in the timing test
REPORT = []
...
def load_data(proxy="gpr"):
    ...
    if sav is None:
        sys.exit("Missing data/ea_saving_rate_quarterly.csv — run 01_collect_data.py first.")
    unc_file = {"gpr": ("gpr.csv", "gpr"), "epu": ("fred_eu_epu.csv", "epu")}[proxy]
    unc = _read(*unc_file)
    if unc is None:
        sys.exit(f"Missing data/{unc_file[0]} — run 01_collect_data.py first.")
...
def _lead_lag(saving, proxy, drop_covid=True, diff=False):
    """dict k -> corr(saving_t, proxy_{t-k}); k>0 = proxy leads saving."""
    ...
    for k in range(-MAXK, MAXK + 1):
        pair = pd.concat([a, b.shift(k)], axis=1).dropna()
...
def proxy_timing():
    say("\n" + "=" * 74)
    say("5) PROXY & TIMING — lead/lag cross-correlation (which proxy, what lag)")
    say("=" * 74)
    sav = _read("ea_saving_rate_quarterly.csv", "saving")
    if sav is None:
        say("  SKIPPED: saving CSV missing.")
        return
    saving = _to_q(sav)
    proxies = {}
    for label, (fname, col) in {"GPR": ("gpr.csv", "gpr"),
                                "EPU": ("fred_eu_epu.csv", "epu")}.items():
        d = _read(fname, col)
        if d is not None:
            proxies[label] = _to_q(d)
    if not proxies:
        say("  SKIPPED: no proxy CSVs (gpr.csv / fred_eu_epu.csv).")
        return

    say("  k>0 = proxy LEADS saving by k quarters; COVID-2020 dropped")
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.6), sharey=True)
    for ax, mode, dif in zip(axes, ("levels", "changes (Δ)"), (False, True)):
        say(f"  == {mode} ==")
        for name, p in proxies.items():
            d = _lead_lag(_z(saving), _z(p), diff=dif)
            k, r = _best_k(d)
            lead = "lead" if k > 0 else ("lag" if k < 0 else "contemp.")
            say(f"    {name:<5} peak |corr| = {r:+.2f} at k={k:+d} ({abs(k)}q {lead})")
            ks = sorted(d)
...
def main():
    ...
    # proxy & timing (compares both proxies regardless of --proxy)
    proxy_timing()
    ...
    say("- Compare full sample vs subsample and GPR vs EPU (--proxy epu) before "
        "settling on the headline. Section 5 reports the best lead/lag per proxy.")
